    ...
    return (
        <>
            ...
            <Grid item container direction='column'>
                ...
                >
                    {
                        props?.category?.map( ( items: { name: string, _id: string } ) =>
                            <MenuItem key={ `${ items?._id }+${items.name}` } value={ items._id }>
                                { items.name }
                            </MenuItem> )
                    }
                </Select>
            ...
            }
            {
                props.edit && <Grid item container justifyContent='center' py='12px'>
                    <Box
                        className={ [ 'width--fit-content position--relative cursor--pointer', 'profile-upload' ].join( ' ' ) }>
                        {
                            image ? <img src={ image }
